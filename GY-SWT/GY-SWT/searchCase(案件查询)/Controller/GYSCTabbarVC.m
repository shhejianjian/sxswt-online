...
@interface GYSCTabbarVC ()

@end

@implementation GYSCTabbarVC
{
    UIButton *_button;
}
- (instancetype)init
...
{
    GYSCDetailFirstVC *view1 = [[GYSCDetailFirstVC alloc]init];
    [self addViewControllers:view1 title:@"网上立案-基本信息"];
    
    GYSCDetailSecondVC *view2 = [[GYSCDetailSecondVC alloc]init];
    [self addViewControllers:view2 title:@"网上立案-当事人信息"];
    
    GYSCDetailThirdVC *view3 = [[GYSCDetailThirdVC alloc]init];
    [self addViewControllers:view3 title:@"网上立案-代理人信息"];
    
    GYSCDetailFourthVC *view4 = [[GYSCDetailFourthVC alloc]init];
...
{
    //  只需要该图片名字就行
    NSArray * normImage = @[@"first",@"second",@"third",@"scfourth"];
    //  只需修改选中图片的名字
    NSArray * selectImage = @[@"first-selected",@"second-selected",@"third-selected",@"scfourth-selected"];
